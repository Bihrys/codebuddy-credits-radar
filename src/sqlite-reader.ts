 ...
 */
export function readValueByKeyMatch(
  filePath: string,
  ...
  const payloadAt = (pg: Buffer, cellOff: number): Buffer => {
    ...
    const chunks: Buffer[] = [Buffer.from(local)];
  ...
  const parseRecord = (payload: Buffer): Array<string | number | undefined> => {
  ...
  const walk = <T>(
    pageNo: number,
    visit: (cols: Array<string | number | undefined>) => T | undefined
  ): T | undefined => {
  ...
  const rootPage = walk<number>(1, (cols) =>
    cols[1] === "ItemTable" ? Number(cols[3]) : undefined
  );
  if (!rootPage) return undefined;

  // ItemTable 的列：(key, value)
  return walk<string>(rootPage, (cols) => {
    const key = cols[0];
