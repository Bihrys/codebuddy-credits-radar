 ...
 * 或在设置里手动补充即可。
 */
function effectivePackageCodes(): string[] {
  const extras = getConfig().get<string[]>("extraPackageCodes", []) ?? [];
 ...
 * 无需等待扩展发新版。
 */
async function detectPackageCodes(): Promise<void> {
  const cfg = getConfig();
  const apiBase = cfg.get<string>("apiBase", "https://www.workbuddy.cn").replace(/\/$/, "");
  try {
    const resp = await authFetch(`${apiBase}/billing/meter/get-user-resource`, {
    ...
    const missing = new Map<string, { name: string; remain: number }>();
    for (const a of accounts) {
      const code = String(a.PackageCode ?? "").trim();
      if (!code || covered.has(code)) continue;
      const remain =
        parseFloat(String(a.CycleCapacityRemainPrecise ?? a.CapacityRemainPrecise ?? "0")) || 0;
    ...
    if (missing.size === 0) {
      void vscode.window.showInformationMessage(
        t("Credits Radar: All packages are already counted")
      );
      return;
    }
    ...
    const items: PackagePick[] = [...missing.entries()].map(([code, v]) => ({
      label: code,
      description: v.name,
    ...
    const existing = (cfg.get<string[]>("extraPackageCodes", []) ?? [])
      .map((s) => String(s ?? "").trim())
      .filter(Boolean);
    ...
    void vscode.window.showInformationMessage(
      t("Credits Radar: Added {0} package code(s), refreshing…", picked.length)
    );
    await update();
  } catch (e) {
    void vscode.window.showWarningMessage(
      t("Credits Radar: Failed to detect package codes ({0})", e instanceof Error ? e.message : String(e))
    );
  }
}
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("codebuddyUsage.detectPackageCodes", () => detectPackageCodes())
  );
  context.subscriptions.push(
