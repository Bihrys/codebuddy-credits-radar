 ...
 */
function credentialFormHtml(webview, cookie, userAgent) {
    const esc = (s) => s
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
...
async function setCookie() {
    const cfg = getConfig();
    const panel = vscode.window.createWebviewPanel("codebuddyUsageCredentials", "设置登录凭证", vscode.ViewColumn.Active, { enableScripts: true, retainContextWhenHidden: false });
    panel.webview.html = credentialFormHtml(panel.webview, cfg.get("cookie", ""), cfg.get("userAgent", ""));
    panel.webview.onDidReceiveMessage(async (msg) => {
        if (msg?.type === "save") {
            await cfg.update("cookie", String(msg.cookie ?? "").trim(), vscode.ConfigurationTarget.Global);
            await cfg.update("userAgent", String(msg.userAgent ?? "").trim(), vscode.ConfigurationTarget.Global);
