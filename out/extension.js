...
function getTokenStore() {
    ...
    if (!tokenStore) {
        ...
        tokenStore = {
            ...
            set: async (value) => {
                if (value)
                    await secrets.store(TOKEN_CACHE_KEY, JSON.stringify(value));
                else
                    await secrets.delete(TOKEN_CACHE_KEY);
            },
        };
    }
    return tokenStore;
...
async function currentAuth() {
    return (0, auth_1.getAuth)({ manualToken: getConfig().get("accessToken", "") });
}
/**
 * 带鉴权的 fetch：
...
async function departBuddy(locationId = 1) {
    try {
        ...
        if (json?.code === 0) {
            ...
            const departHours = json?.data?.duration_hours ?? json?.data?.duration;
...
async function ensureBuddy() {
    const buddyTravel = getConfig().get("buddyTravel", false);
    if (!buddyTravel)
        return undefined;
 ...
 */
function notifyBuddyResult(buddy) {
    if (!buddy)
        return;
    ...
    if (buddy.freshlyClaimed) {
        ...
        vscode.window.showWarningMessage(t("Credits Radar: Failed to claim buddy credits ({0})", claim.error));
    }
    if (buddy.freshlyDeparted && depart?.hours != null) {
        ...
        vscode.window.showWarningMessage(t("Credits Radar: Buddy failed to depart ({0})", depart.error));
    }
    if (parts.length > 0) {
        vscode.window.showInformationMessage(t("Credits Radar: Buddy travel — {0}", parts.join(", ")));
    }
}
async function update() {
    ...
    try {
        ...
        if (autoCheckin && checkin?.freshlyClaimed) {
            vscode.window.showInformationMessage(checkin.credit
            ...
            vscode.window.showWarningMessage(t("Credits Radar: Daily check-in failed, will retry later"));
        }
...
function checkinTag() {
    // 配置关闭时不展示签到标签
    if (!getConfig().get("autoCheckin", true))
        return "";
    // Hover Markdown 不支持 $(icon) 语法，直接用 Unicode 符号表达签到状态
    if (!lastCheckin)
        return t("`❔ Unknown`");
    if (lastCheckin.state === "claimed")
        return t("`✓ Checked in`");
    if (lastCheckin.state === "unclaimed")
...
function buildTooltip(res, updatedAt) {
    ...
    const plansUrl = `${getConfig()
        .get("apiBase", "https://www.workbuddy.cn")
    ...
    if (visible.length === 0) {
        ...
        for (const a of visible.slice(0, 5)) {
            const remain = formatNumber(parseFloat(a.CycleCapacityRemainPrecise ?? a.CapacityRemainPrecise ?? "0"));
            const size = formatNumber(parseFloat(a.CycleCapacitySizePrecise ?? a.CapacitySizePrecise ?? "0"));
...
function scheduleTimer() {
    ...
    const cfg = getConfig();
    const mins = cfg.get("refreshIntervalMinutes", 30);
    if (mins > 0) {
...
async function buddyClaimCmd() {
    ...
    try {
        ...
        if (c.error) {
            ...
            vscode.window.showWarningMessage(t("Credits Radar: Failed to claim buddy credits ({0})", c.error));
        }
        else {
            let credit = c.credit;
            ...
            if ((credit ?? 0) > 0) {
                vscode.window.showInformationMessage(t("Credits Radar: Buddy claimed {0} credits", credit));
            }
            else if (c.claimed) {
                // 已确认领取成功但拿不到数量：不能误报成「没有可领取」
                vscode.window.showInformationMessage(t("Credits Radar: Buddy travel credits claimed"));
            }
            else {
                // 手动点击也要有反馈，否则点了「领积分」没有任何回应
                vscode.window.showInformationMessage(t("Credits Radar: Your buddy has no travel credits to claim right now"));
            }
        }
    }
    catch (e) {
        ...
        vscode.window.showWarningMessage(t("Credits Radar: Failed to claim buddy credits ({0})", e?.message ?? String(e)));
    }
...
async function buddyDepartCmd() {
    ...
    try {
        ...
        if (d.error) {
            ...
            vscode.window.showWarningMessage(t("Credits Radar: Buddy failed to depart ({0})", d.error));
        }
        else {
            lastBuddy.depart = { hours: d.hours ?? 0 };
            vscode.window.showInformationMessage(t("Credits Radar: Buddy departed, travel time {0} hours", d.hours ?? 0));
        }
    }
    catch (e) {
        if (e?.message === "AUTH_EXPIRED" || e?.message === "NO_CREDENTIALS") {
        ...
        vscode.window.showWarningMessage(t("Credits Radar: Buddy failed to depart ({0})", e?.message ?? String(e)));
    }
    const st = await fetchBuddyStatus();
    if (st)
        lastBuddy.status = st;
...
function activate(context) {
    ...
    (0, auth_1.useTokenStore)(getTokenStore());
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeConfiguration((e) => {
        ...
        if (e.affectsConfiguration("codebuddyUsage.accessToken") ||
